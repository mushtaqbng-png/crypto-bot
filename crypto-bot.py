...
# ✅ Predefined popular coins (CoinGecko IDs)
COINS = {
    "Bitcoin": "bitcoin",
    "Ethereum": "ethereum",
...
def get_crypto_price(symbol):
    ...
    print(f"Fetching {symbol} from CoinGecko...")  # 👀 Log request
    try:
        response = requests.get(url, timeout=5)
        print(f"Status: {response.status_code}, Response: {response.text}")  # 👀 Log response
        if response.status_code == 200:
            data = response.json()
            if symbol in data:
                price = data[symbol]["usd"]
                cache[symbol] = (price, time.time())
                return f"💰 *{symbol.capitalize()}* price: `${price:,}` USD"
        return "⚠️ Could not fetch price right now. Please try again later."
